...
<%
//한글 깨짐 처리 - 검색폼에서 입력된 한글이 전송되기 때문
...
여러개의 파라미터가 있는 경우 한꺼번에 저장한 후 DAO로 전달할 용도..
String은 키값으로, Object는 밸류값으로..
*/
...
//파라미터가 많아질 경우 관리가 어려우므로 효율적 관리를 위해 맵에 저장하여 한번에 관리한다.
String searchColumn = request.getParameter("searchColumn");
String searchWord = request.getParameter("searchWord");
if(searchWord != null) {
	//검색어가 있는 경우에만 Map에 값을 저장한다.
	param.put("Column", searchColumn);
	param.put("Word", searchWord);
	
	//검색어가 있을 때 쿼리스트링을 만들어준다. (페이지번호 클릭 때 전달 예정)
	queryStr += "searchColumn="+searchColumn+"&searchWord="+searchWord+"&";
}

//board테이블에 입력된 전체 레코드의 갯수를 카운트하여 반환받음
...
//전체페이지수 계산 ex) 105개 / 10 => 10.5 => ceil(10.5) => 11
int totalPage = (int)Math.ceil((double)totalRecordCount/pageSize);

/*
현재페이지번호: 파라미터가 없을 때는 무조건 1페이지로 지정하고, 
	있을 때는 해당 값을 얻어와서 지정한다. 즉, 리스트에 처음 진입했을 때는 1페이지가 된다.
*/
...
<div class="container">
	...
	<div class="row">		
		...
		<div class="col-9 pt-3">
			...
			<%
			//List컬렉션에 입력된 데이터가 없을 때 true를 반환한다.
			if(bbs.isEmpty()) {
			%>	
				<tr>
			...
			<%
				...
				*/
				...
				현재페이지 1
					첫번재 게시물: 107 - (((1-1)*10)+0) = 107
					두번재 게시물: 107 - (((1-1)*10)+1) = 106
					...
				현재페이지 2
					첫번재 게시물: 107 - (((2-1)*10)+0) = 97
					두번재 게시물: 107 - (((2-1)*10)+1) = 96
					...
				*/
			%>	
