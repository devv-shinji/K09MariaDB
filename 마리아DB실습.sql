...
/*
	...
	지정된 컬럼은 자동으로 값이 증가하게 된다. 
	단 자동증가로 지정되면 데이터를 임의로 삽입할 수 없게된다.
*/
create table board (
...
#member 테이블과 외래키 제약조건이 있으므로 board테이블에 먼저 삽입할 경우 오류 발생
...
/* 빈값은 삽입할 수 없다. 오류발생됨. */
INSERT INTO tb_int (num1, num2, num3, num4, fnum1, fnum2) 
	VALUES ('', 12345, 1234567, 1234567890, 12345.12345, 1234567890.1234567891);
...
/*
	날짜타입의 컬럼에 현재날짜를 입력할 때 오라클은 sysdate를 사용하지만
	MySQL은 now() 함수를 사용한다.
*/
...
#설정된 값중 선택해서 입력
INSERT INTO tb_spec (spec1, spec2) VALUES ('M', 'B,C,A');
#spec1은 디폴트가 설정되었으므로 자동입력
INSERT INTO tb_spec (spec2) VALUES ('B,A');
#설정되지 않은 값으로 입력시 오류발생
INSERT INTO tb_spec (spec1, spec2) VALUES ('A', 'E,C,A');
...
SELECT COUNT(*) FROM board WHERE title LIKE '%제목입니다1%';

-- 페이징처리를 위한 쿼리문 (오라클의 서브쿼리 방식과 다름)
SELECT * FROM board ORDER BY num DESC;

-- 한 페이지에 2개의 게시물이 출력된다고 가정했을 때
/*
	페이지처리를 위해 게시물의 범위를 정할 때 Oracle은 rownum의 속성을 사용하지만
	MariaDB는 limit를 사용한다.
...
INSERT INTO board (title, content, id, bname)
	VALUE ('여긴공지사항', '내용없음', 'kosmo', 'notice');
